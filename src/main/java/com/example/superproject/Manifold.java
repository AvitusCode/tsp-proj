...
public class Manifold {
    ...
    public void posCorr()  {
        ...
        for (Point contact : contacts){
            ...
            double lambda = 1.0 / (t*t + (1-t)*(1-t)) / 2;
            System.out.println(contacts.size());

            if (!flags[B.getIndexPoint(contactSide.getKey())]) {
                contactSide.getKey().setPos(contactSide.getKey().getPos().add(collisionVector.multiply((1 - t) * 0.5 * lambda)));
            }
            else if (!flags[B.getIndexPoint(contactSide.getValue())]) {
                contactSide.getValue().setPos(contactSide.getValue().getPos().add(collisionVector.multiply(t * 0.5 * lambda)));
            }

            flags[B.getIndexPoint(contactSide.getKey())] = true;
            flags[B.getIndexPoint(contactSide.getValue())] = true;
        }
    }
